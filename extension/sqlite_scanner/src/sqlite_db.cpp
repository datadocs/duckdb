...
#ifdef __EMSCRIPTEN__
	// Open through the DuckDB FileSystem-backed VFS so reads/writes hit OPFS and
	// persist (the default emscripten VFS keeps everything in volatile memory).
	// `:memory:` databases don't touch the VFS, so naming it is harmless.
	vfs_name = DuckDBSQLiteVFSName();
#endif
	auto rc = sqlite3_open_v2(path.c_str(), &result.db, flags, vfs_name);
	if (rc != SQLITE_OK) {
...
#ifdef __EMSCRIPTEN__
	// Keep the rollback journal in memory: the DuckDB FileSystem-backed VFS
	// persists the main db file, but creating sidecar journal files for
	// not-pre-registered OPFS paths is unreliable. MEMORY avoids them while
	// still flushing committed pages to the main file (and thus to OPFS).
	if (options.access_mode != AccessMode::READ_ONLY && path != ":memory:") {
		result.Execute("PRAGMA journal_mode=MEMORY");
	}
#endif
	return result;
}
